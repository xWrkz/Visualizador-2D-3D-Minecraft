...
import { switchTo3DMode } from "./visualizer.js";
import { translateBlockName } from './data.js';
...
export async function generatePDF() {
    ...
    // Removed rotation.y = 0 so the PDF respects the user's manual rotation

    let coverAngle = (3 * Math.PI) / 4;
    if (title && title.toLowerCase().includes('espejo')) {
        coverAngle = (7 * Math.PI) / 4;
    }
    window.perspectiveCamera.position.set(radius * Math.cos(coverAngle), camHeight, radius * Math.sin(coverAngle));
    ...
    window.currentModelGroup.traverse(child => {
        ...
        } else if (child.userData.isBlueprintEdge) {
            child.visible = false;
        }
    });
    ...
    // Restore blueprint edges
    window.currentModelGroup.traverse(child => {
        if (child.userData.isBlueprintEdge) {
            child.visible = window.is2DMode === true;
        }
    });
    ...
    for (let i = 1; i <= window.currentModelHeight; i++) {
        ...
        window.currentModelGroup.traverse((child) => {
            if (child.isMesh) {
                ...
                if (isAtLayer) {
                    const rawName = child.userData.englishId || child.userData.trueMinecraftName || child.material.name || child.name || "Bloque";
...
export async function generate3DPDF() {
    if (!window.currentModelGroup || window.currentModelHeight <= 0) return;
    ...
    for (let i = 1; i <= window.currentModelHeight; i++) {
        ...
        for (let a = 0; a < 4; a++) {
            ...
            const angle = angleData.a;
              ...
              // FORCE visibility and position synchronously before render so animate() cannot interfere
              window.currentModelGroup.traverse((child) => {
                  if (child.isMesh && child.userData.box) {
                      child.visible = (child.userData.box.max.y <= i + 0.2);
...
export async function generateInventoryPDF() {
    ...
    window.currentModelGroup.traverse((child) => {
        if (child.isMesh) {
            const rawName = child.userData.englishId || child.userData.trueMinecraftName || child.material.name || child.name || "Bloque Desconocido";
            const cleanName = rawName.replace(/_/g, ' ').replace(/[0-9]/g, '').trim();
            let translatedName = typeof translateBlockName === 'function' ? translateBlockName(cleanName) : cleanName;
            ...
            let finalRawName = cleanName;
            
            if (size.y < 0.2) {
                translatedName += " (Alfombra)";
                if (finalRawName.includes("wool")) {
                    finalRawName = finalRawName.replace("wool", "carpet");
                }
            ...
            if (!inventory[translatedName]) {
                inventory[translatedName] = {
                    ...
                    rawName: finalRawName.replace(/ /g, '_'),
                    meshRef: child
                };
            }
    ...
    for (const item of items) {
        if (item.rawName) {
            const itemImg = await loadImageAsync(`models/items/${item.rawName}.png`);
            if (itemImg) {
